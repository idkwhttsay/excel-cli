...
String_View next_token(String_View *source, const char *file_path, size_t file_row, const char *line_start) 
{
...
Expr_Index parse_primary_expr(String_View *source, Tmp_Cstr *tc, Expr_Buffer *eb, 
    const char *file_path, size_t file_row, const char *line_start) 
{
    String_View token = next_token(source, file_path, file_row, line_start);

    if (token.count == 0) {
        fprintf(stderr, "%s:%zu:%zu: ERROR: expected primary expression token, but got end of input\n", file_path, file_row, source->data - line_start + 1);
        exit(1);
    }
...
Expr_Index parse_plus_expr(String_View *source, Tmp_Cstr *tc, Expr_Buffer *eb,
    const char *file_path, size_t file_row, const char *line_start) 
{
    Expr_Index lhs_index = parse_primary_expr(source, tc, eb, file_path, file_row, line_start);

    String_View token = next_token(source, file_path, file_row, line_start);
    if(token.data != NULL && sv_eq(token, SV("+"))) {
        Expr_Index rhs_index = parse_plus_expr(source, tc, eb, file_path, file_row, line_start);
...
Expr_Index parse_expr(String_View *source, Tmp_Cstr *tc, Expr_Buffer *eb, 
    const char *file_path, size_t file_row, const char *line_start) 
{
    return parse_plus_expr(source, tc, eb, file_path, file_row, line_start);
}
